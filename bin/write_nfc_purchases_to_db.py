...
cmdMap = {
    ...
    "blinkOrangeWithSound":
    [0xFF, 0x00, 0x40, 0xCF, 0x04, 0x00, 0x01, 0x01, 0x02],
    "blinkGreenWithSound":
    [0xFF, 0x00, 0x40, 0b1010110, 0x04, 0x00, 0x01, 0x01, 0x02],
        ...
        for card in addedcards:
            ...
            if res:
                ...
                tag, created = Tag.objects.get_or_create(
                    tag_value=toHexString(res))
                try:
                    purchase = Purchase(
                        tag=tag, date=now(), price=Price.objects.latest('id'))
                ...
                lcd.lcd_clear()
                if tag.owner:
                    rank = int(user_totals['total_coffees'].rank(
                        ascending=False)[tag.owner.id])
                    user = user_totals.loc[tag.owner.id]
                    balance = user['balance']
                    full_name = user['full_name']
                    lcd.lcd_display_string(
                        line=1, string="{}. {}".format(rank, full_name))
                    lcd.lcd_display_string(
                        line=2,
                        string="{}EUR {}".format(balance,
                                                 user["total_coffees"]))
                else:
